...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const action = util_1.getActionInput('action', true, ['put', 'delete']);
            const manifestPath = util_1.getActionInput('manifest_path', true, []);
            const options = {
                enable_update: util_1.getActionInput('enable_update', false, ['true', 'false'], 'true') === 'true',
                enable_permanent_protection: util_1.getActionInput('enable_permanent_protection', false, ['true', 'false'], 'true') === 'true',
            };
            const input = util_1.inputs();
            ...
            const originalManifest = util_1.read(manifestPath);
            util_1.write('.manifest.bak.yaml', originalManifest);
            core.debug(`🔍 File content: ${JSON.stringify(originalManifest)}`);
            core.setOutput('backup', '.manifest.bak.yaml');
            if (action === 'put') {
                const result = util_1.putEnv(input, originalManifest, options);
                util_1.write(manifestPath, result);
                core.debug(`✅ Updated ArgoCD Manifest Environments.`);
                core.debug(`📝 After: ${JSON.stringify(result)}`);
            }
            if (action === 'delete') {
                const result = util_1.deleteEnv(input, originalManifest, options);
                util_1.write(manifestPath, result);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.getEnvIndexOf = exports.getEnv = exports.existsEnv = exports.deleteEnv = exports.putEnv = exports.environments = exports.write = exports.read = exports.inputs = exports.getActionInput = void 0;
...
function getActionInput(key, required, restricted = [], defaults) {
    ...
    if (restricted.length > 0 && !restricted.includes(input)) {
...
exports.write = write;
const environments = (manifest) => manifest.environments;
exports.environments = environments;
const putEnv = (put, manifest, options) => {
    if (!options.enable_update && exports.existsEnv(put.env, put.key, manifest)) {
...
const deleteEnv = (put, manifest, options) => {
    if (!exports.existsEnv(put.env, put.key, manifest)) {
        core.debug(manifest.toString());
        throw new Error(`Environment ${put.env} - ${put.key} not exists.`);
    }
    if (options.enable_permanent_protection && exports.existsEnv(put.env, put.key, manifest) && exports.getEnv(put.env, put.key, manifest).lifecycle === 'permanent') {
        core.debug(manifest.toString());
        throw new Error(`Protected exception [${put.env} - ${put.key}].`);
    }
    const result = clone_1.default(manifest);
    result.environments.splice(exports.getEnvIndexOf(put.env, put.key, result), 1);
    return result;
};
